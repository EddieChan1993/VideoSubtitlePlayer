...
| **播放控制** | 播放/暂停（Space）、回到片头、音量滑块（默认 30%，跨会话记忆） |
| **复制字幕** | 一键复制当前字幕到剪贴板 |
| **语音转字幕** | 工具栏一键调用 whisper-cli 转录视频音频，生成 SRT 字幕并自动加载；支持生成双语字幕（macOS 26+，Translation.framework）；需手动导入 ggml `.bin` 模型文件；识别过程中可随时点叉号取消 |
| **字幕轨道管理** | 内置轨道与外挂轨道均可点击 × 删除；同名轨道自动加「（内置）」后缀区分 |
